# LinK25 拨钟问题
[题目链接](http://xmuoj.com/contest/362/problem/LinK25)

## 思路

每个钟有 $4$ 种操作方式，即顺时针旋转 $0$ 到 $3$ 次。

由于只有 $9$ 个钟，我们可以利用枚举法在规定时间内完成求解。

这里我们依旧使用**状态压缩**进行枚举以简化代码。由于每个钟有 $4$ 种情况，我们应使用 $4$ **进制状态掩码**。掩码范围为 $0 \sim 4^9 - 1$。

---

## 注意

这里我们能简单枚举每个钟操作的次数，是因为我们对不同种的操作是**可以线性叠加的**。
...
**枚举法**处理这题已经足够，但这里提供一种[时间复杂度](https://oi-wiki.org/basic/complexity/)更低的解法，供参考。
...
其中 $A \sim I$ 为初始时各个钟相对于 $0^\circ$ 的旋转次数。
