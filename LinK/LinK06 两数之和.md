...
3. 若对应数的和小于目标，那么需要使和减小，故将 `i` 右移一位
4. 若对应数的和等于目标，即求解成功

相比暴力枚举，双指针将时间复杂度由 $O(n^2)$ 降低至 $O(n)$
...
$$ a_i + a_j < \text{target} ,\nobreakspace a_{i+1} + a_j > \text{target}  $$

由于数列严格单调，则可以得出对于任意的索引 `i` ，我们都无法找到 $ a_i + a_j = \text{target}$ ，这个时候我们必须将索引 `j` 左移一位，否则无法找到解。
