...
1. 我们选择枚举第一列，而不是枚举第一行。这是因为第一列只有 $5$ 盏灯，而第一行有 $6$ 盏灯。尽管对 $6$ 栈灯枚举远不会超时，但枚举 $5$ 盏灯还是更优。
2. 我们选择用**状态掩码**来表示第一列的情况，这样可以用一个循环实现对所有情况的枚举，**使得代码更简介**。*(LinK19 的题解中我们已经介绍过状态掩码，这里不再赘述)*
