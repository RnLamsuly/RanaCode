...
选择基准：从数组中选择一个元素作为基准（这一步时间复杂度为 $O(1)$）

分区：将数组重新排列，使得所有比基准小的元素在基准左边，所有比基准大的元素在基准右边（这一步时间复杂度为 $O(n)$

递归排序：对基准左右两个子数组递归执行上述操作（这一步时间复杂度为 $O(\log n)$
...
$\text{C++}$ 的库 `<algorithm>` 中内置了快速排序函数 `sort`，给定排序区间首位的指针以及比较函数（可选），即可进行快排。
