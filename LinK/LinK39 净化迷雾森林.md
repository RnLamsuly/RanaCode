...
本题我们使用 BFS（广度优先搜索）解决。

与[DFS](https://oi-wiki.org/search/dfs/)不同，BFS 并不追求每次走一条路径到底部。

BFS 通常先将初始状态记录下来，然后每次从记录的状态中选择一个，遍历能从其到达的状态并记录下来，之后把选择的状态给移除掉，并在记录的状态中选择下一个以重复上述步骤，直到记录的所有状态都被遍历。
