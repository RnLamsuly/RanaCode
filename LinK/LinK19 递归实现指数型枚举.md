...
我们可以用一个数字的二进制位来对应拿取数字的方案，也就是**状态掩码**，这种用掩码压缩存储状态信息所需要内存的方式叫做**状态压缩**。

给定一个数字，若其第 $i$ 二进制位上的数是 $1$ 则代表拿取数字 $i+1$，否则则代表不拿(我们规定二进制位从右往左标记，从 $0$ 开始)。这样从 $0$ 至 $2^n-1$ 个数就能代表所有的 $2^n$ 种拿取情况。枚举这些数字即枚举方案。
